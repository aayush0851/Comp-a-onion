...
type NotificationPayload = { postId?: string; userId?: string; reviewerId?: string; joinRequestId?: string; decision?: string };
...
export class NotificationsService {
  ...
  async create(userId: string, kind: NotificationKind, payload: Prisma.InputJsonValue) {
    ...
    this.posts.emit('notification.created', { userId, notification: enriched });
    return notification;
  }
  ...
  // payload only stores ids — resolve the post title and the other person's
  ...
  private async enrich(notifications: Notification[]) {
    ...
    return notifications.map((n) => {
      ...
      return {
        ...n,
        postTitle: p.postId ? (postById.get(p.postId)?.title ?? null) : null,
