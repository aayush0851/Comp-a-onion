...
export function PlanCard({
  ...
  host, hostInitials, hostPhoto, hostRating, hostCount, hostTone = 'peach', compact, onPress, onPressCta, onPressHost,
}: {
  title: string;
  blurb?: string;
  venue?: string | null;
        ...
        <View style={styles.planCardFooter}>
          <View style={{ flex: 1, minWidth: 0 }}>
            <Text style={{ fontFamily: 'Figtree_500Medium', fontSize: 12, color: colors.muted }} numberOfLines={1}>
              {filled > 0 ? `${host} + ${filled} ${filled === 1 ? 'person' : 'people'} signed up for this` : 'Be the first to sign up'}
            </Text>
          </View>
          {!!kind && (
